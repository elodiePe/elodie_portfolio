<template>
    ...
    :href="href || '#'"
        :target="target"
        :rel="computedRel"
        class="inline-flex items-center justify-center px-4 py-2 rounded-full transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
        :class="colorClasses"
        @click="onClick"
    >
        <slot>{{ label }}</slot>
...
function onClick(e) {
    // if the button is just a placeholder (no real href), prevent navigation
    if (!props.href) {
        e.preventDefault()
    }
    emit('click', e)
